...
public class ObjWriter {
    ...
    private static void writePolygons(BufferedWriter writer, List<Polygon> polygons) throws IOException {
        for (Polygon polygon : polygons) {
            ...
            // Чтобы избежать ошибки, добавлена проверка наличия элементов в списках
            // vertexIndices, textureVertexIndices и normalIndices
            // перед тем, как получить доступ к их элементам.
            for (int i = 0; i < vertexIndices.size(); i++) {
                if (i < vertexIndices.size()) {
                    writer.write(vertexIndices.get(i) + 1 + "/");
                }
                if (i < textureVertexIndices.size()) {
                    writer.write(textureVertexIndices.get(i).toString());
                }
